...
#print('svc_cv_score:' , balanced_accuracy_score(ifnrem_2, y_pred_ifnrem))
# extract the index
...
y_pred_mat = np.zeros((x_test_emg_13.shape[0], 7))
...
for k in range(7):
    ...
    nn.add(Conv1D(5, 10, strides=1, activation='relu'))
    nn.add(Dropout(0.3))
    nn.add(MaxPooling1D(2))
    nn.add(LSTM(2, activation ='relu', return_sequences= True, return_state= False))
#    nn.add(Dropout(0.3))
    nn.add(Flatten())
    ...
    nn.fit(nn_train_emg_deleted, ifweak_train_13_trans, epochs=100, verbose=2, batch_size = 500,callbacks=[EarlyStopping(monitor='loss', patience=6)])


    y_pred_mat[:,k] = np.argmax(nn.predict(x_test_emg_13), axis=1)

#print('nn_cv_score:', balanced_accuracy_score(ifweak_2_13, y_pred_ifweak))


y_pred_ifweak = np.zeros(y_pred_mat.shape[0])
for j in range(y_pred_mat.shape[0]):
    y_pred_ifweak[j] =  Counter(y_pred_mat[j]).most_common(1)[0][0]


print('vote_mat:',y_pred_mat[:5])
print('vote_result:', y_pred_ifweak[:5])
...
if sum(results == 0) == 0:
    print('ok')
#    print('final_cv_score:', balanced_accuracy_score(label_2, results))
