...
nn.add(Conv1D(5, 30, strides=1, activation='relu'))
nn.add(Dropout(0.3))
nn.add(Conv1D(5, 30, strides=1, activation='relu'))
nn.add(Dropout(0.3))
nn.add(MaxPooling1D(5))
...
y_pred_ifweak = np.argmax(nn.predict(x_train_2_emg_13), axis=1)
...
results = np.zeros(label_2.shape[0])
